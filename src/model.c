...
//Returns just the pixel RGB of an image
color4ub sample2D(TGAHeader tga_header, color4ub *image, vector2f uv) {
    //normal to screen space
    int x = fmin(fmax(0,(int)(uv.x * (tga_header.width-1))), tga_header.width-1);
    int y = fmin(fmax(0,(int)((1-uv.y) * (tga_header.height-1))), tga_header.height-1);
